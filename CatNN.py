...
class CatNN(nn.Module):
    ...
    def __init__(self, field_size, feature_sizes,
        ...
        if self.use_deep:
            ...
            self.linear_1 = nn.Linear((self.field_size * self.embedding_size)-80, self.deep_layers[0] )
    ...
    def forward(self, X):
        ...
        if self.use_fm:
            Xi = Xi[:, 2:]
            fm_first_order = self.fm_first_order_embedding(Xi)
            if self.is_shallow_dropout:
                fm_first_order = self.fm_first_order_dropout(fm_first_order)
            fm_second_order_emb = self.fm_second_order_embedding(Xi)
        ...
        if self.use_deep:
            if self.use_fm:
                ...
                deep_emb = self.fm_second_order_embedding(Xi)
