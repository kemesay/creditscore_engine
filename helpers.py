        ...
        for group in self.param_groups:
            if self.weight_decay != 0:
                for p in group['params']:
                    if p.grad is None:
                        continue
                    p.data.add_(other=p.data, alpha=-self.weight_decay)
...
def outputFromEmbeddingModel(emb_model, leaf_preds, label_size, n_models):
    ...
    emb_model.bout.weight.data.cpu().numpy() == (n_models*config.config['embsize'], label_size)
    output_w = emb_model.bout.weight.data.cpu().numpy()
    # print(output_w)
