...
import pandas as pd
from helpers import eval_metrics, printMetric
...
def trainModel(model, train_x, train_y, tree_outputs, test_x, test_y, optimizer,
               train_x_cat=None, test_x_cat=None, epochs=20, early_stopping_rounds=5, save_model=True):
    ...
    tree_outputs = torch.tensor(tree_outputs, dtype=torch.float, device=device)
    if train_x_cat is not None:
        train_x_cat = torch.tensor(train_x_cat.values,dtype=torch.float, device=device)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if train_x_cat is not None:
                ...
                # print(type(inputs_cat), 'train-x-cat')
            else:
        ...
        print("Epoch %d: training loss %.4f" % (epoch + 1, running_loss / num_it))
        running_loss = 0.0
...
def evaluateModel(model, test_x, test_y, test_x_cat=None):
    ...
    if test_x_cat is not None:
        test_x_cat = torch.tensor(test_x_cat.values, dtype= torch.float, device= device)
...
def makePredictions(model, test_x, test_cat):
    device = config.config['device']
    test_x = torch.tensor(test_x, dtype=torch.float, device=device)
    test_cat = torch.tensor(test_cat.values, dtype= torch.float, device= device)
    # print(test_x, test_cat, 'testeteteettetetetetestesttestetestestete')
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            max_value = 100
            min_value = 5
            x = ((outputs* (max_value - min_value)) + min_value)[0]
